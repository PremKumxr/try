...
if selected == 'Diabetes Prediction':
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        Pregnancies = st.text_input('Number of Pregnancies', placeholder="e.g., 2", help="Abnormal: >20")
    with col2:
        Glucose = st.text_input('Glucose Level', placeholder="e.g., 120", help="Abnormal: >150 mg/dL")
    with col3:
        BloodPressure = st.text_input('Blood Pressure', placeholder="e.g., 80", help="Abnormal: >150 mmHg")
    with col1:
        SkinThickness = st.text_input('Skin Thickness', placeholder="e.g., 20", help="Used in ML model")
    with col2:
        Insulin = st.text_input('Insulin Level', placeholder="e.g., 80", help="Abnormal: >150 mu U/ml")
    with col3:
        BMI = st.text_input('BMI', placeholder="e.g., 30", help="Used in ML model")
    with col1:
        DiabetesPedigreeFunction = st.text_input('Diabetes Pedigree Function', placeholder="e.g., 0.5", help="Used in ML model")
    with col2:
        Age = st.text_input('Age', placeholder="e.g., 30", help="Used in ML model")

    if st.button('Diabetes Test Result'):
        try:
            user_input = [Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age]
            user_input = [float(x) for x in user_input]
            pregnancies, glucose, bp, skin_thickness, insulin, bmi, dpf, age = user_input
...
if selected == 'Heart Disease Prediction':
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        age = st.text_input('Age', placeholder="e.g., 50", help="Used in ML model")
    with col2:
        sex = st.text_input('Sex', placeholder="1 or 0", help="Used in ML model")
    with col3:
        cp = st.text_input('Chest Pain Type', placeholder="e.g., 0", help="Used in ML model")
    with col1:
        trestbps = st.text_input('Resting Blood Pressure', placeholder="e.g., 120", help="Abnormal: >140 mmHg")
    with col2:
        chol = st.text_input('Cholesterol', placeholder="e.g., 200", help="Abnormal: >240 mg/dl")
    with col3:
        fbs = st.text_input('Fasting Blood Sugar', placeholder="1 or 0", help="Used in ML model")
    with col1:
        restecg = st.text_input('Resting ECG', placeholder="e.g., 0", help="Used in ML model")
    with col2:
        thalach = st.text_input('Max Heart Rate', placeholder="e.g., 150", help="Abnormal: <100 or >180")
    with col3:
        exang = st.text_input('Exercise Induced Angina', placeholder="1 or 0", help="Used in ML model")
    with col1:
        oldpeak = st.text_input('ST Depression', placeholder="e.g., 1.0", help="Abnormal: >2")
    with col2:
        slope = st.text_input('ST Slope', placeholder="e.g., 2", help="Used in ML model")
    with col3:
        ca = st.text_input('Major Vessels', placeholder="e.g., 0", help="Used in ML model")
    with col1:
        thal = st.text_input('Thalassemia', placeholder="e.g., 2", help="Used in ML model")

    if st.button('Heart Disease Test Result'):
        try:
            user_input = [age, sex, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal]
            user_input = [float(x) for x in user_input]
            age, sex, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal = user_input
            ...
            if abnormal_params:
                ...
                # Use ML model for other parameters
                heart_prob = heart_disease_model.predict_proba([user_input])[0][1]
...
if selected == 'Parkinsons Prediction':
    ...
    with col1:
        fo = st.text_input('MDVP:Fo(Hz)', placeholder="e.g., 120", help="Used in ML model")
    with col2:
        fhi = st.text_input('MDVP:Fhi(Hz)', placeholder="e.g., 150", help="Used in ML model")
    with col3:
        flo = st.text_input('MDVP:Flo(Hz)', placeholder="e.g., 100", help="Used in ML model")
    with col4:
        Jitter_percent = st.text_input('MDVP:Jitter(%)', placeholder="e.g., 0.5", help="Abnormal: >0.6%")
    with col5:
        Jitter_Abs = st.text_input('MDVP:Jitter(Abs)', placeholder="e.g., 0.0001", help="Used in ML model")
    with col1:
        RAP = st.text_input('MDVP:RAP', placeholder="e.g., 0.003", help="Used in ML model")
    with col2:
        PPQ = st.text_input('MDVP:PPQ', placeholder="e.g., 0.004", help="Used in ML model")
    with col3:
        DDP = st.text_input('Jitter:DDP', placeholder="e.g., 0.009", help="Used in ML model")
    with col4:
        Shimmer = st.text_input('MDVP:Shimmer', placeholder="e.g., 0.02", help="Abnormal: >0.03")
    with col5:
        Shimmer_dB = st.text_input('MDVP:Shimmer(dB)', placeholder="e.g., 0.2", help="Used in ML model")
    with col1:
        APQ3 = st.text_input('Shimmer:APQ3', placeholder="e.g., 0.01", help="Used in ML model")
    with col2:
        APQ5 = st.text_input('Shimmer:APQ5', placeholder="e.g., 0.012", help="Used in ML model")
    with col3:
        APQ = st.text_input('MDVP:APQ', placeholder="e.g., 0.015", help="Used in ML model")
    with col4:
        DDA = st.text_input('Shimmer:DDA', placeholder="e.g., 0.03", help="Used in ML model")
    with col5:
        NHR = st.text_input('NHR', placeholder="e.g., 0.02", help="Abnormal: >0.03")
    with col1:
        HNR = st.text_input('HNR', placeholder="e.g., 20", help="Used in ML model")
    with col2:
        RPDE = st.text_input('RPDE', placeholder="e.g., 0.5", help="Used in ML model")
    with col3:
        DFA = st.text_input('DFA', placeholder="e.g., 0.7", help="Used in ML model")
    with col4:
        spread1 = st.text_input('spread1', placeholder="e.g., -5", help="Used in ML model")
    with col5:
        spread2 = st.text_input('spread2', placeholder="e.g., 0.2", help="Used in ML model")
    with col1:
        D2 = st.text_input('D2', placeholder="e.g., 2.5", help="Used in ML model")
    with col2:
        PPE = st.text_input('PPE', placeholder="e.g., 0.3", help="Abnormal: >0.4")
    ...
    if st.button("Parkinson's Test Result"):
        try:
            ...
            user_input = [float(x) for x in user_input]
            fo, fhi, flo, jitter_percent, jitter_abs, rap, ppq, ddp, shimmer, shimmer_db, apq3, apq5, apq, dda, nhr, hnr, rpde, dfa, spread1, spread2, d2, ppe = user_input
